public class CueStick {
  ...
  public void render(PoolTable table, Ball target){
    ...
    PVector pointing = relativePos.copy().rotate(PI).div(size).mult(2.5);
    // starting velocity: pointing.mag() = v0
    // rate of change: table.smoothness = s
    
    // pointing.div(1-table.smoothness);
    // total velocity: geometric series, a/(1-r) -> v0/(1-s)
    
    pointing.mult(1-pow(table.smoothness, ceil(log(.1/pointing.mag())/log(table.smoothness)))).div(1-table.smoothness);
    aim = PVector.add(pointing, target.position);
    // total velocity: finite geometric series, a(1-r^n)/(1-r)
  ...
  public void strike(Ball target){
    if(!visible) return;
    PVector relativePos = new PVector(mouseX, mouseY).sub(VISUAL_OFFSET).sub(target.position);
    PVector pointing = relativePos.rotate(PI).div(size).mult(2.5);
