public class CueStick {
  ...
  public void render(Ball target){
    ...
    line(target.position.x+pointing.x*target.size*2, target.position.y+pointing.y*target.size*2,
         target.position.x+pointing.x*size*1.5,      target.position.y+pointing.y*size*1.5);
